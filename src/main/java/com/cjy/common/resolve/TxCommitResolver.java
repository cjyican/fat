...
public class TxCommitResolver {
	 ...
	 * @param param
	 */
	public void blockProceed(TxResolveParam param) {
	...
	public boolean popCommitListWithBlankTime(TxRedisKeyEnum keyEnum ,TxRedisKeyEnum markEnum , String txKey , String content , long waitMilliesSeconds ) {
		...
		throw new TxException(txKey, txKey+" local service is finished , wait for commit timeout");
	}
	 ...
	 * @param param
	 */
	public void clientProcced(TxRegisterService txRegisterService, String remoteTxKey , String localTxKey ,String rootTxKey , String serviceId ){
	 ...
	 */
	public Object waitServiceResult(TxResolveParam param) throws InterruptedException {
		...
		if(StringUtils.isBlank(serviceResult)) {
			...
			if(!isPassed) {
				...
				throw new TxException(param.getTxKey(), "wait result time out , transaction roll back ");
			}
			//不能阻止事务提交，进入等待
			for(;;) {
				//检查是否事务出错
